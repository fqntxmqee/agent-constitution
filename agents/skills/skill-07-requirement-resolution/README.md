...
Skill-06（Requirement Resolution）是**需求解决智能体**的核心执行技能。在「执行蓝图已确认」后，按蓝图中的任务/步骤顺序执行，调用必要的外部工具或 API（含 Cursor CLI），实时上报进度与异常，失败时自动重试或调整策略，并产出代码/文档/数据等交付物。
...
| **核心约束** | 开发类代码仅通过 Cursor CLI（`cursor agent --print`）或 ACP 模式产出，禁止使用 `write` 直接写业务代码 |
...
| 项目 | 路径 |
|------|------|
| **技能根目录** | `agents/skills/skill-06-requirement-resolution/` |
| **入口文件** | `index.js` |
| **规约文档** | `SKILL.md` |
...
```javascript
const { solver } = require('./agents/skills/skill-06-requirement-resolution/index.js');
...
- **开发类任务**：必须通过 **Cursor CLI**（`cursor agent --print`）或 **ACP 模式** 执行；禁止使用 `write` 直接写业务代码（配置文件、文档、脚本等非业务代码允许 write）。
...
- **支持工具名**：如 `cursor-cli`、`exec` 等；当前实现中 `cursor-cli` 与 `exec` 为桩实现，实际集成时需接入真实 Cursor CLI 或 ACP。
- **规则**：
  - 所有外部调用需带超时，失败时进入重试或自我修正流程。
  - 开发类步骤应使用 `cursor-cli` 传入 `task`、`step`、`context`，由上层保证在 ACP 环境中执行。
...
```bash
cd agents/skills/skill-06-requirement-resolution
node test.js
```
...
**Q: 开发类代码为什么不能用 write？**  
A: 宪法 V3.7 要求需求解决阶段仅通过 Cursor CLI（或 ACP）产出开发类代码，以保证可追溯与合规；配置文件、文档等非业务代码允许 write。

**Q: 如何接入真实 Cursor CLI？**  
A: 替换或扩展 `ToolInvoker` 的 `invoke('cursor-cli', ...)` 实现，在 ACP 或本地环境中调用 `cursor agent --print` 并解析输出，返回 `{ success, output, error }`。
