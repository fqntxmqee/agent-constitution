...
| 4 | L1-L5 业务领域归档 | L4 功能点文件 + L5 测试点套件 → `project/{项目}/archive/L1/L2/L3-{BE\|FE}/` |
...
### 铁律（≤3 条）
- ✅ 必须执行交付校验清单
- ✅ 必须完成 L1-L5 业务领域归档：**L4 功能点文件**为核心归档物，L5 测试点套件为附属归档
- ✅ 必须获得用户二次确认才能执行生产部署
...
**说明**: 
- 快速响应：C 级任务（简单交付，无归档）
- 标准响应：A/B/S 级任务（含交付校验 + L1-L5 归档 + Git 提交）
- 超时降级：返回「处理中」+ 异步完成
...
- 归档目录：`project/{项目名}/archive/{L1 领域}/{L2 场景}/{L3-BE 活动 或 L3-FE 页面}/`
  - L4 归档：`{L4 功能点}.md`（核心归档物）
  - L5 归档：`test-suite.md`（附属归档，按 L3 聚合的测试点套件）

### 必需字段
- 交付物清单
- 交付校验结果
- 归档完成确认：L4 功能点文件已创建（必需）+ L5 测试点套件已更新（如有新增测试点）
- 用户确认记录
- Git 提交/部署记录
...
└── archive/                       # 知识归档目录（交付智能体负责）
    └── {L1 领域}/                 # 业务领域
        └── {L2 场景}/             # 业务场景
            ├── {L3-BE 活动}/      # 后端业务活动
            │   ├── {L4 功能点}.md        # 功能点归档文件
            │   └── test-suite.md         # L5 测试点套件
            └── {L3-FE 页面}/      # 前端页面/组件
                ├── {L4 UI模块}.md        # UI 模块归档文件
                └── test-suite.md         # L5 测试点套件
```
...
---
**配置状态**: ✅ V3.17.0 已生效  
**最后更新**: 2026-04-11（L1-L5 归档结构升级：L3 拆分 BE/FE + L5 测试点套件）
