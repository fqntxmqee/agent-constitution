...
**更新日期**: 2026-04-05  
**状态**: 已生效  
**关联规范**: `agents/docs/specs/constitution/CONSTITUTION.md` (V3.18.0)
...
| Gate 编号 | 检查点 | 前置阶段 | 后置阶段 | 检查时机 | 适用流程 |
|-----------|--------|----------|----------|----------|----------|
| Gate #1 | 澄清→理解 | Clarification | Understanding | 澄清完成后 | 复杂任务 |
| Gate #1S | 澄清→解决 | Clarification | Resolution | 澄清完成后 | 简单任务 |
| Gate #2 | 理解→解决 | Understanding | Resolution | 理解完成后 | 复杂任务 |
| Gate #3 | 解决→验收 | Resolution | Acceptance | 解决完成后 | 全部 |
| Gate #4 | 验收→交付 | Acceptance | Delivery | 验收完成后 | 全部 |
...
```
┌─────────────┐      ┌─────────────┐     ┌─────────────┐     ┌─────────────┐
│ Clarification│─────▶│  Resolution │────▶│ Acceptance  │────▶│   Delivery  │
│   (澄清)    │ G1S  │   (解决)    │ G3  │   (验收)    │ G4  │   (交付)    │
└─────────────┘      └─────────────┘     └─────────────┘     └─────────────┘
```

未通过时退回前一阶段整改（同一 Gate 最多 3 次）。
...
| 2 | L1-L5 定位已完成（含 L3-BE/L3-FE 归属） | 检查报告中包含 L1-L5 定位内容 | ✅ |
...
### 3.1S Gate #1S: 澄清→解决（简单任务）

**检查时机**: 简单任务（B/C 级）澄清完成后，跳过理解阶段直接进入解决前

**检查清单**:

| 序号 | 检查项 | 检查方法 | 必需 |
|------|--------|----------|------|
| 1 | 轻量执行计划已生成 | 检查 `.tasks/{task-id}/proposal.md` 存在 | ✅ |
| 2 | 用户确认已获取 | 检查报告中有用户确认记录 | ✅ |
| 3 | 复杂度评级为 B/C 级 | 检查评级结果，排除误判降级 | ✅ |

**检查清单 (Markdown 格式)**:
```markdown
### Gate #1S: Clarification → Resolution (Simple)
- [ ] 轻量执行计划已生成 (proposal.md)
- [ ] 用户确认已获取
- [ ] 复杂度评级为 B/C 级
```
...
**不通过处理**: 退回 Clarification 阶段；若复杂度评估有误（实际需 OpenSpec），升级为复杂任务流走 Gate #1
...
| 5 | L5 验收级测试点已生成 | 检查 acceptance-criteria.md 中包含 L5 测试点（绑定 L3 的验收标准） | ✅ |
| 6 | 用户确认已获取 | 检查 OpenSpec 文档有用户确认记录 | ✅ |
...
| 4 | L5 契约测试通过 | 检查 L5-Contract 测试执行结果（绑定 L4 的功能契约测试） | ✅ |
| 5 | 代码审查通过 (如适用) | 检查代码审查记录 (若有代码变更) | ⚠️ |
...
- **检查清单**:
  - [x] 意图识别报告已生成
  - [x] L1-L5 定位已完成（含 L3-BE/L3-FE 归属）
  - [x] 用户确认已获取
  - [x] HARD GATE 检查通过
- **备注**: [如有]
  ...
  "gate_records": [
    {
      ...
      "checklist": [
        ...
        { "item": "L1-L5 定位已完成（含 L3-BE/L3-FE 归属）", "passed": true },
...
银河导航员 (`agents/constitution/GALAXY_NAVIGATOR.md`) 作为 Hub 执行 Gate 检查（属于协调职责，非业务执行）：

1. **Gate 检查触发**: 阶段流转前自动触发 Hard Gate 检查
2. **检查执行**: 按照检查清单逐项检查（检查文件是否存在、字段是否完整）
3. **流转控制**: 未通过 Gate 时阻止流转
4. **记录持久化**: 检查结果写入 gate-records.md

**说明**: Gate 检查是 Hub 的协调性工作（验证产出物完整性），不涉及业务代码的读写或执行，因此不违反「Hub 禁止直接执行业务任务」的铁律。
...
| V3.18.0 | 2026-04-11 | L1-L5 升级：Gate #1 增加 L3-BE/FE 归属检查；Gate #2 增加 L5 验收标准检查；Gate #3 增加 L5 契约测试检查 | 银河导航员 |
